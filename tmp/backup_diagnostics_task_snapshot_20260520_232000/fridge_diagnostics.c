...
esp_err_t fridge_diagnostics_get_status(fridge_device_status_t *status)
{
    ...
    status->tasks[0] = (fridge_task_snapshot_t){ "main_task", "高", "running", "boot" };
    status->tasks[1] = (fridge_task_snapshot_t){ "usb_protocol", "中", "running", "stdin" };
    status->tasks[2] = (fridge_task_snapshot_t){ "wifi", "中", net.connected ? "connected" : "idle", net.ip };
    status->task_count = 3;
    return ESP_OK;
}
