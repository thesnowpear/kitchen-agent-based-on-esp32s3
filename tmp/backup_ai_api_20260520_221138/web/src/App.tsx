...
function App() {
  ...
  const refreshAll = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const [statusRes, networkRes, pinsRes, sensorsRes, diagnosticsRes, logsRes] =
        await Promise.all([
  ...
  const saveNetwork = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      appendLog("info", "网络配置已发送。第三方 AI Key 不会写入设备。", "network");
      await refreshAll();
    } catch (error) {
  ...
  return (
    <form className="section-flow" onSubmit={saveNetwork}>
      ...
      <div className="wifi-console">
        ...
        <div className="wifi-connect-panel">
          ...
          <button className="advanced-toggle" type="button" onClick={() => setShowAdvanced((value) => !value)}>
            {showAdvanced ? "收起高级参数" : "展开 MQTT / API 高级参数"}
          </button>
        </div>
      </div>
      ...
      {showAdvanced && (
        ...
        <label>
          <span>云端 API</span>
          <input value={current.apiBaseUrl} required onChange={(event) => update("apiBaseUrl", event.target.value)} />
        </label>
        <label>
          <span>NTP 服务器</span>
      ...
      <div className="warning-line">
        <Info size={16} />
        像手机联网一样先选 Wi-Fi、输密码、点连接。设备端仍不得保存第三方大模型 API Key，AI 服务凭证只放在云端 AI Adapter。
      </div>
    </form>
  );
}
