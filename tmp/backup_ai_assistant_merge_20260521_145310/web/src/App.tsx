...
const sections: SectionDefinition[] = [
  ...
  { id: "project_ai", label: "项目 AI", icon: Sparkles },
...
function App() {
  ...
  const [projectAiResult, setProjectAiResult] = useState<ProjectAITaskResponse | null>(null);
  ...
  const runProjectAiTask = async () => {
    ...
    try {
      ...
      setProjectAiResult(result.payload);
...
function AiPanel({
  ...
  return (
    <div className="section-flow">
      ...
      <div className="warning-line">
        <Info size={16} />
        本页是开发期单次 API 连通性测试：每次只发送当前输入和系统提示词，不保存多轮聊天上下文。需要测试库存、记忆和菜谱等项目上下文时，请使用“项目 AI”页面。
      </div>
        ...
        <div className="ai-config-main">
          <div className="form-grid">
            <label>
              <span>配置名称</span>
              <input
                value={current.profileName ?? "默认配置"}
                maxLength={32}
          ...
          <label>
            ...
            <textarea
              ...
              placeholder="可输入较长角色设定、回答风格和限制条件，保存前会按 UTF-8 字节检查。"
              onChange={(event) => update({ systemPrompt: event.target.value })}
            />
            <small className={systemPromptTooLong ? "form-hint danger" : "form-hint"}>
              {systemPromptBytes}/{AI_SYSTEM_PROMPT_MAX_BYTES} UTF-8 字节；较长提示词会增加每次 AI 请求的 token 和延迟。
            </small>
          </label>
          <div className="button-row">
            ...
            <button
              className="action-button secondary danger-soft"
              type="button"
              disabled={!connected || (current.profileId ?? 0) === 0}
              onClick={() => void deleteAiProfile(current.profileId ?? 0)}
            >
              <Trash2 size={17} />
              删除配置
            </button>
      ...
      <form className="ai-chat-panel" onSubmit={(event) => void sendAiChat(event)}>
        <div className="ai-chat-head">
          <div>
            <h3>模拟聊天测试</h3>
            <p>发送一条短消息，设备会调用 OpenAI-compatible /chat/completions；这里不会自动带上上一轮聊天。</p>
          </div>
...
function ProjectAiPanel({
  ...
  connection: ConnectionState;
  request: ProjectAITaskRequest;
  setRequest: (request: ProjectAITaskRequest) => void;
  preview: AIContextPreview | null;
  result: ProjectAITaskResponse | null;
  ...
  const connected = connection === "connected";
  const update = (patch: Partial<ProjectAITaskRequest>) => setRequest({ ...request, ...patch });
  const contextBytes = utf8ByteLength(JSON.stringify(preview?.context ?? {}, null, 2));
  const resultBytes = utf8ByteLength(JSON.stringify(result?.result ?? {}, null, 2));

  return (
    <div className="section-flow">
      <div className="section-heading">
        <div>
          <p className="eyebrow">AI Orchestrator / Context Injection</p>
          <h2>项目 AI 任务测试</h2>
        </div>
        <StatusPill state={busy ? "warn" : connected ? "ok" : "offline"}>{busy ? "生成中" : connected ? "可预览" : "未连接"}</StatusPill>
      </div>

      <div className="warning-line">
        <Info size={16} />
        这里测试正式项目 AI 链路的“任务 + 最小上下文 + 结构化结果”，不会直接调用第三方模型，也不会修改库存。
      </div>

      <div className="project-ai-grid">
        <form className="project-ai-card" onSubmit={(event) => { event.preventDefault(); void runTask(); }}>
          <h3>任务输入</h3>
          <label>
            <span>任务类型</span>
            <select value={request.taskType} onChange={(event) => update({ taskType: event.target.value as ProjectAITaskType })}>
          ...
          <label>
            <span>用户输入 / 调试文本</span>
            <textarea
              value={request.userText}
              rows={4}
              maxLength={512}
              placeholder="例如：今晚想用快过期的食材做一道 30 分钟以内的菜。"
              onChange={(event) => update({ userText: event.target.value })}
            />
          </label>
          <div className="context-toggle-grid">
            <label><input type="checkbox" checked={request.includeInventory} onChange={(event) => update({ includeInventory: event.target.checked })} />库存快照</label>
            <label><input type="checkbox" checked={request.includeReminders} onChange={(event) => update({ includeReminders: event.target.checked })} />临期提醒</label>
            <label><input type="checkbox" checked={request.includePreferences} onChange={(event) => update({ includePreferences: event.target.checked })} />用户偏好</label>
            <label><input type="checkbox" checked={request.includeMemory} onChange={(event) => update({ includeMemory: event.target.checked })} />记忆摘要</label>
          </div>
          <div className="button-row">
            <button className="action-button secondary" type="button" disabled={!connected || busy} onClick={() => void previewContext()}>
              <Search size={17} />
              预览上下文
            </button>
            <button className="action-button" type="submit" disabled={!connected || busy}>
              <Sparkles size={17} />
              测试项目 AI
            </button>
          </div>
        ...
        <div className="project-ai-card">
          ...
          <pre className="json-preview">{JSON.stringify(memorySummary ?? { note: "尚未读取。记忆只保存结构化摘要，不保存完整聊天记录。" }, null, 2)}</pre>
        </div>
      </div>

      <div className="project-ai-grid wide">
        <div className="project-ai-card">
          <div className="ai-chat-head compact-head">
            <h3>上下文预览</h3>
            <StatusPill state={preview?.needsConfirmation ? "warn" : preview ? "ok" : "offline"}>
              {preview ? `${contextBytes} bytes` : "未生成"}
            </StatusPill>
          </div>
          <pre className="json-preview tall">{JSON.stringify(preview?.context ?? { note: "点击“预览上下文”查看本次任务会注入哪些数据。" }, null, 2)}</pre>
        </div>

        <div className="project-ai-card">
          <div className="ai-chat-head compact-head">
            <h3>Mock 结构化结果</h3>
            <StatusPill state={result?.needsConfirmation ? "warn" : result ? "ok" : "offline"}>
              {result ? `${result.confidence}% / ${resultBytes} bytes` : "未测试"}
            </StatusPill>
          </div>
          {result?.safetyNote && (
            <div className="warning-line">
          ...
          <pre className="json-preview tall">{JSON.stringify(result?.result ?? { note: "点击“测试项目 AI”查看 Schema 化返回；真实识别结果仍需用户确认。" }, null, 2)}</pre>
        </div>
      </div>
    </div>
  );
