...
端侧职责是稳定、可解释、可离线，并负责直接访问 AI API。后续如需要跨设备同步、远程日志或更强 Key 管理，可以再增加云端服务，但不作为首版 AI 主链路。
...
设备端直接感知 OpenAI-compatible 接口，不再通过云端 AI Adapter 中转。首版最小接口：
...
- `mock`：比赛现场或离线演示兜底。
- `openai_compatible`：正式首版真实模型接入方式。
...
- 已新增 `cloud/ai_adapter_mock`，现作为可选参考后端保留，不再是首版 AI 主链路。
