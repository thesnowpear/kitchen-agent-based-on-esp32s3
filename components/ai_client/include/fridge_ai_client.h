...
#define FRIDGE_AI_MAX_SYSTEM_PROMPT_LEN 8192
#define FRIDGE_AI_MAX_CHAT_MESSAGE_LEN 768
#define FRIDGE_AI_MAX_CHAT_HISTORY 30
#define FRIDGE_AI_MAX_CONTEXT_LEN 8192
...
// 注意：首版采用设备直连 OpenAI-compatible API，API Key 保存在 NVS；串口响应和日志不得回显明文。
typedef struct {
    uint8_t profile_id;
...
// AI 配置列表：最多保存少量设备直连配置，便于在不同 OpenAI-compatible 服务之间切换。
typedef struct {
...
// AI 助手历史消息：只接收 Web 面板裁剪后的最近几轮，不在设备端长期保存。
typedef struct {
    char role[12];
    char content[FRIDGE_AI_MAX_CHAT_MESSAGE_LEN + 1];
} fridge_ai_chat_history_item_t;

// AI 助手请求：ESP32-S3 直连 OpenAI-compatible API，用于正式首版上下文注入链路。
typedef struct {
    char message[FRIDGE_AI_MAX_CHAT_MESSAGE_LEN + 1];
    char task_type[32];
    char context_json[FRIDGE_AI_MAX_CONTEXT_LEN + 1];
    const fridge_ai_chat_history_item_t *history;
    size_t history_count;
    bool context_injected;
    bool needs_confirmation;
    uint32_t local_snapshot_version;
} fridge_ai_assistant_request_t;

// AI 助手结果：复用聊天结果字段，并附带项目上下文元数据，供 Web 面板展示。
typedef struct {
